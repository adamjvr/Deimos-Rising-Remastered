...
int main() {
    ...
    spawn.x = 123.25f;
    spawn.y = 234.5f;
    spawn.player_owner_index = 3;
    d = deimos::apply_collision_damage(spawn, spawn_u, 1.0f, -1, 2, rng, 0);
    assert(d.collision_spawn_due && *d.collision_spawn_due == id("boom"));
    assert(d.collision_spawn_request);
    const auto& collision_request = *d.collision_spawn_request;
    assert(collision_request.unit_id == id("boom"));
    assert(near(collision_request.x, 123.25f));
    assert(near(collision_request.y, 234.5f));
    assert(collision_request.player_owner_index == 3);
    assert(collision_request.parent.handle == spawn.handle);
    assert(collision_request.parent.serial == spawn.serial);
    assert(!collision_request.subtract_world_y_origin);
    assert(!collision_request.heading_is_set);
    assert(collision_request.heading_degrees == 0);
    assert(collision_request.editor_heading_degrees == 0);
    assert(!collision_request.stationary);
    assert(!collision_request.terrain_effects_enabled);
    assert(near(collision_request.initial_velocity_multiplier, 1.0f));
