...
- live-world gameplay slice: Level-1 placement activation now follows the recovered terrain-row scheduler instead of instantiating the entire level at boot; only the two placements reached by the initial `sourceTop-64` row stream are live initially, with the third placement activating at tick 36. All five Weapon Definitions compile into a level-aware player weapon catalog, PlayerWorld slot 0 is authoritative, state rules receive original-shaped Unit-ID/range/global world facts, and the recovered collision -> destruction-effects -> outer-removal chain is connected in the 30 Hz session. The 120-tick canonical soak allocates 24 members, peaks at 20 active, finalizes 7 removals / 10 removal consequences / 1 consequence spawn, and retains integration frame witnesses `0x864f27d9c3820d7f` / `0xe94cfb91faa42e72` / `0x8a8f770b4de4d2cb`; see `docs/LIVE_WORLD_WEAPON_INTEGRATION.md`.
