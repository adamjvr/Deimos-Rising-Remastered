...
`RENDER_ORCHESTRATION_RUNTIME.md` now closes the semantic-to-raw request bridge for the recovered entity/player sprite paths, including frame resolution, world/HUD/terrain coordinate selection, effect-color packing, immediate/queued routing, and the exact `0x100B0` horizontal-view stepper. Terrain/background surface lifetime and scrolling are now reconstructed in `TERRAIN_SURFACE_RUNTIME.md`: the persistent full raster is copied as a complete 416x480 viewport, not incremental strips. Remaining work is to bind the proven top-level frame-loop sequencing and replace legacy display/presentation ownership without changing the proven compositor arithmetic.
